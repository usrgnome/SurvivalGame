...
import { activeVisibleDecorations, addDecoration, deactiveVisibleDecorations, initDecoration } from "./Decoration.ts/Decoration";
...
mapData.FORREST.polygons.forEach(poly => {

  for(let i = 0; i < poly.length; i += 2){
    let cx = poly[i + 0];
    let cy = poly[i + 1];
    let ex = poly[(i + 2) % poly.length];
    let ey = poly[(i + 3) % poly.length];
    ...
    for(let i = 0; i < len; i += step){
      const posX = cx + xDir * i;
      const posY = cy + yDir * i;
      addDecoration(posX, posY, SPRITE.SAND);
    }
...
export function GameClient_update(now: number, delta: number) {
  ...
  const color = numberToHexStr(lerpColor(0x288ccc, 0x288ccc, v));
