...
export default function QuadNode(bound: IBound, maxChildren: number, maxLevel: number, level?: any, parent?: any) {
    if (!level) level = 0;
...
QuadNode.prototype.find = function (bound, result: any[]) {
    if (this.childNodes !== null) {
        var quad = this.getQuad(bound);
        if (quad !== -1) {
            this.childNodes[quad].find(bound, result);
        } else {
            for (var i = 0; i < this.childNodes.length; i++) {
                var node = this.childNodes[i];
                if (checkBoundIntersection(node.bound, bound))
                    node.find(bound, result);
            }
        }
    }
    for (var i = 0; i < this.items.length; i++) {
