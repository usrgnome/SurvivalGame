        ...
        this.server = App()
            .ws('/*', {
                ...
                close(ws, code, message) {
                    if (ws.client) ws.client.onSocketClose(code, message)
                },
                message(ws, message, isBinary) {
                    if (ws.client) ws.client.onSocketMessage(message, isBinary)
                },
                upgrade(res, req, context) {
                    // log some connection info to help debug
                    const headerObject: any = {}
                    req.forEach(
                        (header, value) => (headerObject[header] = value)
                    )
            ...
            .listen(port, (listenSocket) => {
                if (listenSocket)
                    logger.log(
                        loggerLevel.info,
                        `GameServer: listening on port: ${port}`
                    )
                else
                    logger.log(
                        loggerLevel.error,
                        `GameServer: error on port: ${port}`
                    )
            })
    }
}
