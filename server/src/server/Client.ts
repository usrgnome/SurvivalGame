...
import {
    ...
    BufferSchema,
    BufferWriter,
} from '../../../shared/lib/StreamUtils'
import { ITEM, Items } from '../../../shared/Item'
import { removeComponent } from 'bitecs'
import {
    Inventory_canAddItem,
    Inventory_craftItem,
...
const requestRespawnSchema = new BufferSchema([BinaryTypes.str], {
    errorMessage: 'invalid respawn packet!',
})
const inputSchema = new BufferSchema([BinaryTypes.u8, BinaryTypes.f32], {
    errorMessage: 'invalid input packet!',
})
const inventorySchema = new BufferSchema([BinaryTypes.u8], {
    errorMessage: 'invalid invetory packet!',
})
const chatSchema = new BufferSchema([BinaryTypes.str], {
    errorMessage: 'invalid chat packet!',
})
const pongSchema = new BufferSchema([BinaryTypes.u8], {
    errorMessage: 'invalid pong packet!',
})
const craftSchema = new BufferSchema([BinaryTypes.u8], {
    errorMessage: 'invalid craft packet!',
})
...
export class Client {
    ...
    buildSnapshot() {
        ...
        for (let i = 0; i < bodies.length; i++) {
            const body = bodies[i]
            const eid = (<any>body).eid as number
            if (eid === -1) continue
    ...
    onSocketMessage(data: ArrayBuffer, isBinary: boolean) {
        ...
        let currentReads = 0
        while (inStream.hasMoreData() && currentReads++ < 10) {
            // dont read more then N packets crammed together
            const header = inStream.readU8()
            switch (header) {
                case CLIENT_HEADER.CRAFT: {
                    try {
                        craftSchema.validate(inStream)
                    } catch (e) {
                        return
                    }
                ...
                case CLIENT_HEADER.REQUEST_RESPAWN:
                    try {
                        requestRespawnSchema.validate(inStream)
                    } catch (e) {
                        return
                    }
                ...
                case CLIENT_HEADER.INVENTORY: {
                    try {
                        inventorySchema.validate(inStream)
                    } catch (e) {
                        return
                    }
                    ...
                    if (slotOffset >= C_Inventory.items[this.eid].length)
                        console.warn('Too long slot offset')
                    ...
                    if (item.isTool) {
                        ...
                        if (Inventory_removeItem(this.eid, itemId, 1)) {
                            const newHunger = Math.min(
                                C_Hunger.hunger[eid] + 5,
                                C_Hunger.maxHunger[eid]
                            )
    ...
    onInput() {
        ...
        const invMag = x || y ? 1 / Math.hypot(x, y) : 0
        x *= invMag
        y *= invMag
    ...
    onRequestSpawn() {
        ...
        clientDebugLogger.log(
            loggerLevel.info,
            'NICKNAME: ' + nickname + ' DEBUG_INFO: ' + this.socket.debugInfo
        )
    }
