    ...
    if (!this.intervalErrorChecked) {
        ...
        if (!interval) {
            ...
            if (typeof interval != 'string') {
                console.log(
                    "Interval argument to setInterval must be a string specified as an integer followed by 's' for seconds, 'm' for milli, 'u' for micro, and 'n' for nanoseconds. Ex. 2u"
                )
                process.exit(1)
            }
        }
        ...
        if (this.intervalType == 's') {
            this.intervalTime =
                interval.slice(0, interval.length - 1) * 1000000000
            ...
            console.log(
                'Error with argument: ' +
                    interval +
                    ': Incorrect interval format. Format is an integer followed by "s" for seconds, "m" for milli, "u" for micro, and "n" for nanoseconds. Ex. 2u'
            )
            process.exit(1)
        }
        ...
        if (this.difTime < this.intervalTime * this.intervalCount) {
            ...
            if (this.intervalTime > 25000000) {
                if (this.deferredInterval == false) {
                    ...
                    const msDelay = (this.intervalTime - 25000000) / 1000000.0
            ...
            if (this.logging) {
                console.log(
                    'nanotimer log: ' + 'cycle time at - ' + this.difTime
                )
            }
...
NanoTimer.prototype.setTimeout = function (task, args, delay, callback) {
    ...
    if (!task) {
        ...
        if (typeof task != 'function') {
            console.log(
                'Task argument to setTimeout must be a function reference'
            )
            process.exit(1)
        }
    }
    ...
    if (!delay) {
        ...
        if (typeof delay != 'string') {
            console.log(
                "Delay argument to setTimeout must be a string specified as an integer followed by 's' for seconds, 'm' for milli, 'u' for micro, and 'n' for nanoseconds. Ex. 2u"
            )
            process.exit(1)
        }
    }
    ...
    if (delayType == 's') {
        ...
        console.log(
            'Error with argument: ' +
                delay +
                ': Incorrect delay format. Format is an integer followed by "s" for seconds, "m" for milli, "u" for micro, and "n" for nanoseconds. Ex. 2u'
        )
        process.exit(1)
    }
    ...
    if (difTime < delayTime) {
        ...
        if (delayTime > 25000000) {
            if (this.deferredTimeout == false) {
                ...
                const msDelay = (delayTime - 25000000) / 1000000.0
