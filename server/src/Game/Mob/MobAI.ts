...
export function tickPassive(world: World, eid: number, delta: number) {
        ...
        case MOB_STATE.SEARCH_ENTITY:
            ...
            for (let i = 0; i < query.length; i++) {
                ...
                if (
                    queryEid === eid ||
                    queryEid == -1 ||
                    !C_Base.active[queryEid] ||
                    !C_Base.active[queryEid] ||
                    C_Base.type[queryEid] !== types.PLAYER
                )
                    continue
            ...
            if (playersFound.length) {
                ...
                for (let i = 0; i < playersFound.length; i++) {
                    ...
                    if (
                        distSqrd < nearestDist &&
                        distSqrd <= viewDistance * viewDistance
                    ) {
    ...
    C_Rotation.rotation[eid] = lerpAngle(
        C_Rotation.rotation[eid],
        C_Mob.targetAngle[eid],
        0.4
    )
}
