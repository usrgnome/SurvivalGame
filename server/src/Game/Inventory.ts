...
export function Inventory_indexOfItem(
    entity: number,
    itemIdToSearchFor: number
) {
...
export function Inventory_canAddItem(
    entity: number,
    itemIdToInsert,
    insertQuantity = 0
): boolean {
    ...
    for (let i = 0; i < invItems.length; i++) {
        const itemId = invItems[i]
        const quantity = invQuantities[i]
        if (itemId === itemIdToInsert && quantity + insertQuantity < 0xffff)
            return true
...
export function Inventory_tryGiveItem(
    entity: number,
    itemToAdd: number,
...
export function Inventory_hasItem(
    entity: number,
    itemIdToSearchFor: number,
    quantityToSearchFor: number
) {
    const invItems = C_Inventory.items[entity]
    const invQuantities = C_Inventory.quantities[entity]
    for (let i = 0; i < invItems.length; i++) {
        const itemId = invItems[i]
        const quantity = invQuantities[i]
        if (itemId === itemIdToSearchFor && quantity >= quantityToSearchFor)
            return true
    }
}

export function Inventory_removeStack(
    entity: number,
    itemIdToSearchFor: number
) {
    const invItems = C_Inventory.items[entity]
...
export function Inventory_removeItem(
    entity: number,
    itemIdToSearchFor: number,
    quantityToRemove = 1
) {
...
export function Inventory_craftItem(entity: number, itemId: number) {
    ...
    assert(
        entity !== NULL_ENTITY && Number.isInteger(entity),
        'Inventory_canCraft invalid entity id'
    )
