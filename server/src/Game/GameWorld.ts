...
export default class GameWorld extends EventEmitter {
     ...
     * @memberof GameWorld
     */
    bodyMap: Map<number, BodyEntity> = new Map()
    ...
    _on(
        ...
        callback: (e: IEventInventoryChange) => void
    ): void
    ...
    constructor() {
        ...
        Events.on(this.engine, 'collisionStart', function (e) {
            for (let i = 0; i < e.pairs.length; i++) {
                ...
                const bodyA = pair.bodyA as BodyEntity
                const bodyB = pair.bodyB as BodyEntity
                ...
                // look for hash match
                switch (typeHash) {
                    case COLLISION_TYPES.OCEAN |
                        COLLISION_TYPES.LAND_CREATURE: {
                        let creature = bodyA
                        ...
                        assert(
                            eid !== -1 && Number.isInteger(eid),
                            '@ collisionStart, entity eid is not valid!'
                        )
                    ...
                    case COLLISION_TYPES.LAND | COLLISION_TYPES.LAND_CREATURE: {
                        ...
                        if (aTypeMask === COLLISION_TYPES.LAND) creature = bodyB
                    ...
                    case COLLISION_TYPES.SNOW | COLLISION_TYPES.LAND_CREATURE: {
                        ...
                        if (aTypeMask === COLLISION_TYPES.SNOW) creature = bodyB
                    ...
                    case COLLISION_TYPES.LAVA | COLLISION_TYPES.LAND_CREATURE: {
                        ...
                        if (aTypeMask === COLLISION_TYPES.LAVA) creature = bodyB
            ...
            for (let i = 0; i < e.pairs.length; i++) {
                ...
                switch (typeHash) {
                    case COLLISION_TYPES.OCEAN |
                        ...
                        assert(
                            eid !== -1 && Number.isInteger(eid),
                            "@ collisionEnd' entity eid is not valid!"
                        )
                        C_TerrainInfo.inWaterCount[eid]--
                        break
                    }
    ...
    private loadForType(
        type: keyof typeof mapData,
        label: keyof typeof COLLISION_TYPES,
        category: number,
        mask: number,
        sensor: boolean,
        isStatic = false
    ...
    canPlaceStructure(type: number, x: number, y: number) {
        ...
        for (let i = 0; i < query.length; i++) {
            ...
            if (
                otherBody.collisionFilter.category & body.collisionFilter.mask
            ) {
                count++
            }
        }
    ...
    private onEntityKilled(target: number, killer: number) {
        if (
            hasComponent(this.world, C_Leaderboard, killer) &&
            hasComponent(this.world, C_GivesScore, target)
        ) {
    ...
    setHealCooldown(eid: number, cooldown: number) {
        C_Health.healCoolDown[eid] = Math.max(
            cooldown,
            C_Health.healCoolDown[eid]
        )
    }

    heal(eid: number, heal: number, originEntity: number = NULL_ENTITY) {
        assert(
            eid !== NULL_ENTITY && Number.isInteger(eid),
            'World::damage Invalid target eid'
        )
        assert(
            Number.isInteger(originEntity),
            'World::damage Invalid dealer eid'
        )
        assert(
            heal >= 0,
            'Health must be a positive number, use .damage(eid, health) to reduce entity health'
        )
        ...
        if (hasComponent(this.world, C_Health, eid)) {
            ...
            const newHealth = Math.min(
                C_Health.maxHealth[eid],
                Math.floor(health + heal)
            )
    ...
    damage(eid: number, damage: number, originEntity: number = NULL_ENTITY) {
        ...
        assert(
            damage >= 0,
            'Damage must be a positive number, use .heal(eid, health) to increase entity health'
        )
        ...
        if (
            ...
            const didAddItem = Inventory_tryGiveItem(
                originEntity,
                C_GivesResource.resource[eid],
                C_GivesResource.quantity[eid]
            )
    ...
    sweepAttack(
        ...
        assert(
            ...
            'GameWorld::sweep expects targetEid to be integer!'
        )
        ...
        for (let i = 0; i < bodies.length; i++) {
            const body = bodies[i] as BodyEntity

            const targetEid = body.eid as number
            ...
            const sepperationOffset =
                (dealerBody && dealerBody.type === 'circle'
                    ? (dealerBody as any).radius
                    : 0) + (body.type === 'circle' ? (<any>body).radius : 0)

            if (
                distSqrd - sepperationOffset * sepperationOffset >
                range * range
            )
                continue
            ...
            if (
                dealer !== -1 &&
                hasComponent(this.world, C_Leaderboard, dealer) &&
                hasComponent(this.world, C_GivesScore, targetEid)
            )
    ...
    update(delta: number) {
        ...
        debug &&
            console.log(
                'Ecs tick took: ' + (ecs2 - ecs1 + (ecs4 - ecs3)) + 'ms'
            )
    ...
    generateMap() {
        ...
        wall1.collisionFilter.category =
            wall2.collisionFilter.category =
            wall3.collisionFilter.category =
            wall4.collisionFilter.category =
                collisionLayer.ALL
        wall1.collisionFilter.mask =
            wall2.collisionFilter.mask =
            wall3.collisionFilter.mask =
            wall4.collisionFilter.mask =
                collisionLayer.ALL
